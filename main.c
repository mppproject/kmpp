...
void doo(char *file,char *name,int argc){
  ...
  if(strstr(file,".c") == 0){
    if (argc > 2){
      sprintf(last,"gcc %s.c /usr/lib/libmpp/libmpplinux.a -o %s",file,name);
      system(last);
   }
    else {
      sprintf(last,"gcc %s.c /usr/lib/libmpp/libmpplinux.a",file);
      system(last);
    }
  }
  else {
     if (argc > 2){
      sprintf(last,"gcc %s /usr/lib/libmpp/libmpplinux.a -o %s",file,name);
      system(last);
   }
    else {
      sprintf(last,"gcc %s /usr/lib/libmpp/libmpplinux.a",file);
     system(last);
    }
  }
